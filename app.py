...
# Set up logging to debug 404 errors and functionality
...
try:
    ...
    logger.info("Starting Flask app with template_folder='templates' and static_folder='static'")
...
class BookGenerator:
    ...
    def add_character(self, name, age, gender, occupation, personality, backstory, physical_traits, motivations, relationships):
        logger.debug(f"Adding character: {name}")
    ...
    def set_book_details(self, genre, setting, tone, themes, page_count, chapter_count, conflict_intensity, title, key_plot_points):
        logger.debug(f"Setting book details: {title}")
...
"""
    ...
    def preview_book_structure(self):
        logger.debug("Previewing book structure")
...
"""
    ...
    def generate_book(self):
        logger.debug("Generating full book")
        if not self.plot or not self.characters or not self.genre:
            return "Error: Please add characters, set book details, and generate a plot first."
...
def index():
    ...
    try:
        ...
        flash(f"Error rendering homepage: {e}", "error")
        return "Error rendering homepage", 500

@app.route("/add_character", methods=["GET", "POST"])
def add_character():
    logger.debug("Accessing add_character route")
    if request.method == "POST":
        try:
            ...
            logger.debug("Character added successfully")
            return redirect(url_for("index"))
        
        except ValueError as e:
            logger.error(f"Validation error in add_character: {e}")
            flash(f"Error: {e}", "error")
        except Exception as e:
            logger.error(f"Unexpected error in add_character: {e}")
            flash(f"Error: Invalid input. {e}", "error")
    
    try:
        return render_template("add_character.html")
    except Exception as e:
        logger.error(f"Error rendering add_character.html: {e}")
        flash(f"Error rendering add character page: {e}", "error")
        return "Error rendering add character page", 500

@app.route("/set_details", methods=["GET", "POST"])
def set_details():
    logger.debug("Accessing set_details route")
    if request.method == "POST":
        try:
            ...
            session["generator"] = generator.__dict__
            flash(output, "success")
            logger.debug("Book details set successfully")
            return redirect(url_for("index"))
        
        except ValueError as e:
            logger.error(f"Validation error in set_details: {e}")
            flash(f"Error: {e}", "error")
        except Exception as e:
            logger.error(f"Unexpected error in set_details: {e}")
            flash(f"Error: Invalid input. {e}", "error")
    
    try:
        return render_template("set_details.html")
    except Exception as e:
        logger.error(f"Error rendering set_details.html: {e}")
        flash(f"Error rendering set details page: {e}", "error")
        return "Error rendering set details page", 500

@app.route("/generate_plot", methods=["POST"])
def generate_plot():
    logger.debug("Accessing generate_plot route")
    try:
        if not session.get("generator"):
            logger.error("No generator in session")
...
def preview_structure():
    logger.debug("Accessing preview_structure route")
    try:
        if not session.get("generator"):
            logger.error("No generator in session")
            flash("Error: No book data available.", "error")
        ...
        session["generator"] = generator.__dict__
        flash(output, "success")
        logger.debug("Structure preview generated successfully")
        return redirect(url_for("index"))
    except Exception as e:
        logger.error(f"Error in preview_structure: {e}")
        flash(f"Error: {e}", "error")
        return redirect(url_for("index"))

@app.route("/generate_book", methods=["POST"])
def generate_book():
    logger.debug("Accessing generate_book route")
    try:
        if not session.get("generator"):
            logger.error("No generator in session")
            flash("Error: No book data available.", "error")
        ...
        session["generator"] = generator.__dict__
        logger.debug("Book generated successfully, redirecting to book_display")
...
@app.route("/book_display", methods=["GET"])
def book_display():
    logger.debug("Accessing book_display route")
    try:
        ...
        logger.error(f"Error rendering book_display.html: {e}")
        flash(f"Error rendering book display page: {e}", "error")
        return "Error rendering book display page", 500

@app.route("/download_book", methods=["GET"])
def download_book():
    logger.debug("Accessing download_book route")
    try:
        book_content = session.get("book_content", "")
        if not book_content:
            logger.error("No book content available for download")
        ...
        logger.debug(f"Downloading book: {filename}")
