...
## Setup (Local)
Follow these steps to run the app locally, fix 404 errors, and ensure all buttons and functions work.
...
7. **Commit to GitHub** (Optional):
   - Run:
     ```bash
     git add .
     git commit -m "Fix 404 errors and ensure button functionality"
...
## Troubleshooting 404 Errors and Functionality
...
- **Terminal**:
  ...
  - Look for:
    ...
    ```
    Solution: Verify `templates/add_character.html`.
- **Files**:
  - Run:
    ```bash
    find . -type f
...
- **Functionality**:
  ...
  - Ensure session persists (e.g., `session["generator"]` logs).
- **Still Failing**:
  - Provide:
